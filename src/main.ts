...
function getDefinitionInfo(source: string) {
...
function main() {
    rmdir(dir, function(error){ console.log(error) });
    setTimeout(() => {
        fs.mkdirSync(dir);
        ...
        Promise.map(groups, (group) => {
            return Promise.map(group, (target: string) => {
                return getDefinitionInfo(target)
                    .then(normalizeObjectData)
                    .then(writeDefinitionInfoFile);
            });
        });
    }, 500);
}
